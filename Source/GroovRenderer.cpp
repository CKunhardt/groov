...
void GroovRenderer::loadShaders()
{
	const auto& s = getShader();
	const auto& sSky = getSkyShader();

	newVertexShader = s.vertexShader;
	newFragmentShader = s.fragmentShader;

	newVertexSkyShader = sSky.vertexShader;
	newFragmentSkyShader = sSky.fragmentShader;
}
